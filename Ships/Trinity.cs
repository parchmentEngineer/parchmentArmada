...
namespace parchmentArmada.Ships
{
    internal class Trinity : ISpriteManifest, IArtifactManifest, ICardManifest, IDeckManifest, IGlossaryManifest, IShipPartManifest, IShipManifest, IStartershipManifest
    {
        ...
        public string Name => "parchmentEngineer.parchmentArmada.Manifest";
        ...
        {
            artifacts.Add(name, new ExternalArtifact(artifact, "parchment.armada.trinity." + name, sprites[art], null, new ExternalGlossary[0]));
            artifacts[name].AddLocalisation("en", name, desc);
        ...
        private void TrivengeLogic(Harmony harmony)
        {
            ...
            var damage_pre = typeof(Trinity).GetMethod("TrivengeNormalDamagePatch", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic) ?? throw new Exception("Couldnt find TrinityManifest.TrivengeNormalDamagePatch method");
            harmony.Patch(damage_method, prefix: new HarmonyMethod(damage_pre));

            var target_method_2 = typeof(ArtifactReward).GetMethod("GetBlockedArtifacts", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
            var target_patch_2 = typeof(Trinity).GetMethod("AllBossArtifactLogic", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        ...
        private static bool TrivengeNormalDamagePatch(Ship __instance, State s, Combat c, int incomingDamage, int worldGridX, bool piercing = false, bool worldSpaceAgnostic = false)
        {
            foreach(Artifact artifact in s.artifacts)
            {
                if(artifact.Name() == "TRI-VENGE" && __instance.isPlayerShip)
                {
                    Part part = s.ship.GetPartAtWorldX(worldGridX) ?? throw new Exception("Where'd the ship go?");
                    if (part != null)
                    {
        ...
        {
            TrinityStarterCard = new ExternalCard("parchment.armada.TrinityStarterCard", typeof(Cards.TrinityStarterCard), ExternalSprite.GetRaw((int)Spr.cards_GoatDrone), TrinityDeck);
            TrinityStarterCard.AddLocalisation("Assault Bracing");
            registry.RegisterCard(TrinityStarterCard);

            TrinityRagnarok = new ExternalCard("parchment.armada.trinity.ragnarok", typeof(Cards.TrinityRagnarok), ExternalSprite.GetRaw((int)Spr.cards_GoatDrone), TrinityDeck);
